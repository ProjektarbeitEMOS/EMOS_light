...
# ---------------------------------------------------------------------------
# 3-Speicher-Modell (ETH Zuerich, Juni 2026) — Wandknoten T_W
# ---------------------------------------------------------------------------

def test_wall_anchoring_keeps_steady_state_u(lehrbeispiel: Building):
    """Verankerte Wand: Reihen-U von (k_RW, k_WA) == u_value_wall·A_W.

    Damit bleibt der stationaere Gesamtverlust identisch zum alten
    Einzel-UA-Modell (Wandbeitrag = U_Wand·A_Wand), waehrend die Wand
    nur zusaetzliche Traegheit einbringt.
    """
    g_rw = lehrbeispiel.wall_conductance_rw_w_per_k
    g_wa = lehrbeispiel.wall_conductance_wa_w_per_k
    series = 1.0 / (1.0 / g_rw + 1.0 / g_wa)
    expected = lehrbeispiel.u_value_wall * lehrbeispiel.wall_area_net_m2
    assert series == pytest.approx(expected, rel=1e-6)  # 0.2 * 375 = 75 W/K
    # Verhaeltnis der Halbleitwerte folgt k_RW:k_WA der Gebaeudegruppe.
    assert g_rw / g_wa == pytest.approx(
        lehrbeispiel.wall_k_rw_w_m2_k / lehrbeispiel.wall_k_wa_w_m2_k, rel=1e-6
    )


def test_ua_direct_plus_wall_equals_total(lehrbeispiel: Building):
    """UA_direkt (Fenster+Dach+Lueftung) + Wandtransmission == total_UA."""
    wall_transmission = (
        lehrbeispiel.wall_area_net_m2 * lehrbeispiel.u_value_wall
    )
    assert (
        lehrbeispiel.ua_direct_w_per_k + wall_transmission
        == pytest.approx(lehrbeispiel.total_ua_w_per_k)
    )
    assert lehrbeispiel.ua_direct_w_per_k == pytest.approx(427.5)  # 502.5 - 75


def test_room_capacity_is_air_only(lehrbeispiel: Building):
    """C_R im 3-Speicher-Modell = nur Luft (Wandmasse sitzt im T_W-Knoten)."""
    assert (
        lehrbeispiel.room_capacity_kwh_per_k
        == pytest.approx(lehrbeispiel.air_capacity_kwh_per_k)
    )
    # Wandmasse ist NICHT mehr Teil des Raumknotens (kein Doppelzaehlen).
    assert (
        lehrbeispiel.room_capacity_kwh_per_k
        < lehrbeispiel.shell_capacity_kwh_per_k
    )


def test_wall_raw_k_without_anchor():
    """Ohne Ankerung werden die rohen k-Werte der Gebaeudegruppe benutzt."""
    b = Building("raw", {
        "enabled": True, "heated_area_m2": 150,
        "length_m": 15.0, "width_m": 10.0, "height_m": 10.0,
        "window_area_m2": 125.0, "u_value_wall_w_m2_k": 0.2,
        "wall_k_rw_w_m2_k": 2.5, "wall_k_wa_w_m2_k": 25.0,
        "wall_anchor_to_u_value": False,
    })
    assert b.wall_conductance_rw_w_per_k == pytest.approx(2.5 * 375.0)
    assert b.wall_conductance_wa_w_per_k == pytest.approx(25.0 * 375.0)
